...
public class PopupManager : MonoBehaviour {
    [SerializeField] List<GameObject> popupOptions;

    public void HideAllPopups() {
        foreach (var popupOption in popupOptions) {
            if (!popupOption) continue;
    ...
    public void ShowBuildingPopup(int hp, Vector2 pos) {
        HideAllPopups();
        popupOptions[0].transform.Find("BuildingHP").GetComponent<TextMeshProUGUI>().text = $"HP: {hp}/12";
        popupOptions[0].transform.position = pos;
        popupOptions[0].gameObject.SetActive(true);
    }

    public void ShowOffensiveTurretBuildingPopup(string turret_name, int turret_lv, int bhp, float atk_bonus,
        float range_bonus, float cd_bonus, Vector2 pos) {
        HideAllPopups();
        ...
        popupOptions[1].transform.Find("BuildingHP").GetComponent<TextMeshProUGUI>().text = $"HP: {bhp}/12";
        popupOptions[1].transform.Find("BonusList").GetComponent<TextMeshProUGUI>().text =
            $"+{System.Math.Round(atk_bonus * 100.0, 1)}% attack damage\n+{System.Math.Round(range_bonus * 100.0, 1)}% attack range\n-{System.Math.Round(cd_bonus * 100.0, 1)}% cooldown\n";
    ...
    public void ShowDefensiveTurretBuildingPopup(string turret_name, int turret_lv, int bhp, Vector2 pos) {
        ...
        popupOptions[3].transform.Find("BuildingHP").GetComponent<TextMeshProUGUI>().text = $"HP: {bhp}/12";
